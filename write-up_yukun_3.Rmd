...
$W_2$ : race: asian/pacific island, black, white, hispanic

$W_3$ : gender : female 1 or male 0
...
dat_prep <- function(dat, loc, years){
  ...
  filter(year(date) %in% years) %>%  # select year
  ...
  filter(subject_race != 'other' & subject_race != 'unknown') %>% 
  mutate(subject_race = as.character(subject_race)) %>% # change subject_race to string
  mutate(subject_sex = as.numeric(subject_sex) - 1) %>%  # change subject_sex to numeric
...
# relabel race asian/pacific islander
  datBos$subject_race[datBos$subject_race == 'asian/pacific islander'] = 'asian'
# reason_for_stop represents speed or not
  ...
  datBos$reason_for_stop[is.na(datBos$reason_for_stop)] = 'Not Speed'
  return(datBos)
}
...
# data for Boston in 2012 and 2013
datBos <- dat_prep(dat, 'BOSTON', years = c(2012:2013))
...
c(mean(cv.error.1), mean(cv.error.2)) # similar performance, model 1 is simpler
...
for (i in 1:10){
  ...
  cv.error.3[i] <- cv.glm(datBos, fit.prob.D.1, K = 10)$delta[1]
  # model 2
  fit.prob.D.2 <- glm(search_conducted ~ 
                    ...
                    subject_sex*reason_for_stop,
                  family = 'binomial', data = datBos)
  cv.error.4[i] <- cv.glm(datBos, fit.prob.D.2, K = 10)$delta[1]
  print(i)
}
c(mean(cv.error.3), mean(cv.error.4)) # performace are close, model 1 is simpler
...
CV.fit.SL.1 <- CV.SuperLearner(Y=as.numeric(datBos.searched$contraband_found), 
                       X=X, SL.library=SL.library, family='binomial')
summary(CV.fit.SL.1) # superlearner is the best!!!!!!!!!!
...
X.2 <- subset(datBos, select = c(subject_age, subject_race, 
                               subject_sex, reason_for_stop))
fit.SL.2 <- SuperLearner(Y=as.numeric(datBos$search_conducted), 
                       X=X.2, SL.library=SL.library, family='binomial')
# performace of superlearner
CV.fit.SL.2 <- CV.SuperLearner(Y=as.numeric(datBos$search_conducted), 
                       X=X.2, SL.library=SL.library, family='binomial')
summary(CV.fit.SL.2) # all models have similar performance
...
logitUpdate <- glm(datBos.searched$contraband_found ~ -1 + 
                     offset(qlogis(EY.1W.s)) + H.AW.std.s, 
                    family='binomial')
...
# Influence curve
EY.AW <- predict(fit.SL.1, newdata=X)$pred
EY.AW.star <- plogis(qlogis(EY.AW) + epsilon*H.AW.std.s)
Y <- as.numeric(datBos$contraband_found[!is.na(datBos$contraband_found)])
IC <- H.AW.std.s*(Y - EY.AW.star) + EY.1W.star[!is.na(datBos$contraband_found)] - est.TMLE
n.searched <- nrow(datBos.searched)
# variance
varHat.IC <- var(IC)/n.searched
# confidence interval
c(est.TMLE - 1.96*sqrt(varHat.IC), est.TMLE + 1.96*sqrt(varHat.IC))
...
est.all <- function(dat.bp){
  ...
  # IPTW
  X2 <- subset(dat.bp, select = c(subject_age, subject_race, 
                                 subject_sex, reason_for_stop))
  fit.SL.2 <- SuperLearner(Y=as.numeric(dat.bp$search_conducted), 
                         X=X2, SL.library=SL.library, family='binomial')
...
##########################################
# bootstrap: about 3 hours to run
##########################################
# set.seed(2019)
...
# Hit Rate
```{r}
# use asian, white, black, hispanic for race: srtring
# use 1 for female, 0 for male : numeric
hit.rate <- function(dat, race, sex){
  # Simple substitution
  dat.s <-  dat %>% filter(search_conducted == TRUE)
  dat.i <- dat %>% mutate(search_conducted = TRUE, 
  ...
  # IPTW
  X2 <- subset(dat, select = c(subject_age, subject_race, 
                                 subject_sex, reason_for_stop))
  fit.SL.2 <- SuperLearner(Y=as.numeric(dat.bp$search_conducted), 
                         X=X2, SL.library=SL.library, family='binomial')

  X3 <- X2 %>% mutate(subject_race = race, subject_sex = sex)
  PA1.W <- predict(fit.SL.2, newdata = X3)$pred
  H.AW <- as.numeric(dat$search_conducted == 1)/PA1.W
  H.AW.mean.s <- mean(H.AW[!is.na(dat$contraband_found)])
  # standardized IPTW
  est.sIPTW.SL <- mean(H.AW*dat$contraband_found, na.rm = TRUE)/H.AW.mean.s
  
  # TMLE
  H.AW.std.s <- H.AW[!is.na(dat$contraband_found)]/H.AW.mean.s
  EY.1W.s <- EY.1W[!is.na(dat$contraband_found)]
  logitUpdate <- glm(dat.s$contraband_found ~ -1 + 
                       offset(qlogis(EY.1W.s)) + H.AW.std.s, 
                       family='binomial')
  epsilon <- logitUpdate$coefficients
  H.1W <- 1/PA1.W
  H.1W.std <- H.1W/mean(H.1W)
  EY.1W.star <- plogis(qlogis(EY.1W) + epsilon*H.1W.std)
  est.TMLE <- mean(EY.1W.star)
  prob.D.strata <- mean(dat$search_conducted[dat$subject_sex == sex &
                                             dat$subject_race == race])
  est.TMLE/prob.D.strata
}

set.seed(2019)
for (i in c('asian', 'black', 'white', 'hispanic')){
  for (j in c(0, 1)){
    if (j == 0) gender = 'male'
    if (j == 1) gender = 'female'
    print(paste(i, gender))
    print(hit.rate(datBos, race = i, sex = j))
  }
}
# black female 63.72716
# balck male 22.20644
# white female 106.4469
# white male 66.2654
# asian male 67.833
# asian female 76.84243
# hispanic male 16.0246
# hispanic female 26.77237
```
